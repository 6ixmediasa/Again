...
public sealed class WindowsConnector:IConnector {
 ...
 public Task ExecuteAsync(Step step,Context context,CancellationToken token){token.ThrowIfCancellationRequested();var target=step.Target??throw new TargetMissingException("Choose an application target.");
  ...
  switch(step.Operation){case Operation.Click:
...
public sealed class ApplicationProcessor(IEnumerable<IConnector> connectors,Func<Step,Exception,Task<string>> repair):IItemProcessor {
 public async Task<string> ProcessAsync(Workflow workflow,string input,int index,IReadOnlyDictionary<string,string> variables,PauseGate pause,CancellationToken token){
  ...
  foreach(var step in workflow.Steps.Where(x=>x.Enabled)){await pause.WaitAsync(token);if(!Conditions.Evaluate(step.When,values))continue;var connector=connectors.FirstOrDefault(c=>c.Supports(step))??throw new InvalidDataException("Enable a connector for "+step.Name);int attempt=0;
   while(true){try{using var timeout=CancellationTokenSource.CreateLinkedTokenSource(token);timeout.CancelAfter(TimeSpan.FromSeconds(step.TimeoutSeconds));await connector.ExecuteAsync(step,context,timeout.Token);break;}catch(Exception e)when(e is not OperationCanceledException||!token.IsCancellationRequested){if(attempt++<step.Retries)continue;var choice=await repair(step,e);if(choice=="retry")continue;if(choice=="skip")break;throw new InvalidDataException("The workflow stopped at: "+step.Name);}}
  }
