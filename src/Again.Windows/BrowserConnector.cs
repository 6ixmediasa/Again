...
public sealed class BrowserConnector:IConnector,IAsyncDisposable {
 ...
 public async Task ExecuteAsync(Step s,Context c,CancellationToken token){token.ThrowIfCancellationRequested();var p=await Page();p.SetDefaultTimeout(s.TimeoutSeconds*1000);
  ...
  if(s.Target?.Domain is not {Length:>0} domain||!Safety.SameDomain(domain,p.Url))throw new TargetMissingException("The browser domain changed. Review the address before continuing.");
  var element=Find(p,s);switch(s.Operation){case Operation.BrowserClick:await element.ClickAsync();break;
