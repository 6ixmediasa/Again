...
public static class WorkflowDetector
{
    public static WorkflowDetectionResult Detect(ImageFileInfo source, ImageFileInfo output)
    {
        ...
        var provisionalMode = source.Width == output.Width && source.Height == output.Height
            ? ImageGeometryMode.PreserveOriginal
            : ImageGeometryMode.ResizeToFixedSize;
        ...
        var workflow = new WorkflowDefinition(
            ...
            source.Path,
            output.Path,
            new ImageResizeStep(output.Width, output.Height, GeometryMode: provisionalMode),
        ...
        var note = template == "{stem}" && !string.Equals(sourceStem, outputStem, StringComparison.OrdinalIgnoreCase)
            ...
            : "Workflow output, format, destination and naming intent detected.";
