...
public sealed class ImageEngine
{
    public async Task<Image<Rgba32>> RenderAsync(string input, IEnumerable<Step> steps, IReadOnlyDictionary<string, string> variables, PauseGate? pause = null, CancellationToken token = default)
    {
        ...
        try
        {
            image.Mutate(c => c.AutoOrient()); var values = new Dictionary<string, string>(variables) { ["width"] = image.Width.ToString(), ["height"] = image.Height.ToString() };
            foreach (var step in steps.Where(s => s.Enabled))
            {
                ...
                switch (step.Operation)
                {
                    ...
                    case Operation.Logo:
                        ...
                        {
                            int lw = Math.Clamp((int)(image.Width * step.Number("scale", .2)), 1, image.Width); logo.Mutate(c => c.Resize(new ResizeOptions { Size = new(lw, image.Height), Mode = ResizeMode.Max })); var x = (int)((image.Width - logo.Width) * Math.Clamp(step.Number("x", .9), 0, 1)); var y = (int)((image.Height - logo.Height) * Math.Clamp(step.Number("y", .9), 0, 1)); image.Mutate(c => c.DrawImage(logo, new Point(x, y), (float)Math.Clamp(step.Number("opacity", 1), 0, 1)));
                        }
                        break;
